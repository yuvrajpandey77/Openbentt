...
They can **describe the document in the main composer** (the chat input). You should respond with **complete, valid `.tex`** when they want a PDF book or paper—ideally in **one markdown fenced code block** whose language tag is **latex** (triple backticks + latex on the opening fence) so the app can unwrap it. They do **not** need to paste by hand: after your reply, they click **Apply reply** in the Notebook panel to copy that output into **Source**, **run Compile**, and switch to **Preview** automatically. **Last reply** + **Review** is optional for line-by-line merge before compile.

**Source modes**
- **Full LaTeX** (starts with \`\\documentclass\` or a standard preamble + \`\\begin{document}\`): output is a **real PDF** via **pdflatex** (run \`npm run latex-compile\` locally with TeX Live, or set \`VITE_LATEX_COMPILE_URL\` in production). Use valid book/article LaTeX: \\chapter, \\section, lists, titlepage, \\tableofcontents, etc.
