/**
 * Hugging Face token storage via Electron safeStorage when available.
 */
...
const FILE_NAME_ENCRYPTED = "hf_token.blob";
const FILE_NAME_FALLBACK = "hf_token.secret";

/**
 * @param {import('electron').App} app
 */
export function hfSecretPaths(app) {
 ...
 * @param {import('electron').App} app
 */
export async function readHfTokenMaybe(app) {
  const paths = hfSecretPaths(app);
  try {
    ...
    console.warn("[hfSecret] decrypt failed:", e);
  }
  try {
 ...
 * @param {import('electron').App} app
 */
export async function writeHfToken(app, raw) {
  const paths = hfSecretPaths(app);
  ...
  console.warn("[hfSecret] safeStorage unavailable; writing restricted-permission plaintext fallback.");
 ...
 * @param {import('electron').App} app
 */
export async function hfHasStoredToken(app) {
  const t = await readHfTokenMaybe(app);
 ...
 * @param {import('electron').App} app
 */
export function registerHfSecretIpc(ipcMain, app) {
  ipcMain.handle("hfSecret:status", async () => {
    const stored = await hfHasStoredToken(app);
    return {
      stored,
  ...
  ipcMain.handle("hfSecret:set", async (_e, token) => {
    if (typeof token !== "string") {
      await writeHfToken(app, "");
      return { ok: true };
    }
    await writeHfToken(app, token);
    return { ok: true };
  });

  ipcMain.handle("hfSecret:clear", async () => {
    await writeHfToken(app, "");
    return { ok: true };
  });
}
